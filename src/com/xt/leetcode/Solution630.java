 ...
 * 题解：
 * 这题我自己写的方法超时了，最终是参考官方的题解做出来的。
 * courList:用户选择的最优队列。
 * 接来下的每一步都求其最优解。
 * 首先做一个过滤，不符合条件的过滤掉（花费时间>结束时间的）
 * 第二，按照结束时间取排序。因为同样花费时间，结束时间越早的，被选择的优先级就应该越高。
 * 第三，依次获取每一个值，如果已花费时间+当前花费时间<=结束时间，则可以加入到队列当中。
 * 第四，如果已花费时间+当前花费时间>结束时间，则说明当前课程无法加入到队列中，这时候我们就要求最优解了，我们遍历之前的队列，找出比队列中花费时间更少的，然后替换掉队列中的。
 * 这样，我们已花费时间就可以相应的减少两者的差值。同时当前课程会被舍弃掉。
 * 第五，遍历队列的时候，为了减少时间复杂度，所以采用二分插入法。即保证每次插入后都是有序的，花费时间最少的在队列的最前面
 * <p>
 * state:done
 */
