 ...
 * 解题思路：
 * 首先使用\n分割，逐行统计。
 * 使用一个栈来存储当前文件的父目录的路径长度。而当前文件或者文件夹的长度，就是父目录的路径长度+当前文件或文件夹的路径长度。
 * 如果当前路径的文件\t的数量小于栈中的数量，则说明属于更上一层文件夹，则栈执行pop操作。
 * 如果当前路径是一个文件，那么我们计算栈的最上层长度+当前文件路径长度，就可以算出来当前文件的路径总长度。
 * 如果当前路径是文件夹，则我们计算当前文件夹路径长度后加入到栈中。
 * state:done
 */
